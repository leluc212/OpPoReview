...
Write-Host "`nStep 1: Creating DynamoDB Table..." -ForegroundColor Yellow
...
if ($tableCheck) {
    ...
    $gsiJson = '[{\"IndexName\":\"CandidateIndex\",\"KeySchema\":[{\"AttributeName\":\"candidateId\",\"KeyType\":\"HASH\"},{\"AttributeName\":\"appliedAt\",\"KeyType\":\"RANGE\"}],\"Projection\":{\"ProjectionType\":\"ALL\"},\"ProvisionedThroughput\":{\"ReadCapacityUnits\":5,\"WriteCapacityUnits\":5}},{\"IndexName\":\"JobIndex\",\"KeySchema\":[{\"AttributeName\":\"jobId\",\"KeyType\":\"HASH\"},{\"AttributeName\":\"appliedAt\",\"KeyType\":\"RANGE\"}],\"Projection\":{\"ProjectionType\":\"ALL\"},\"ProvisionedThroughput\":{\"ReadCapacityUnits\":5,\"WriteCapacityUnits\":5}}]'
    
    aws dynamodb create-table --table-name $TABLE_NAME --attribute-definitions AttributeName=applicationId,AttributeType=S AttributeName=candidateId,AttributeType=S AttributeName=jobId,AttributeType=S AttributeName=appliedAt,AttributeType=S --key-schema AttributeName=applicationId,KeyType=HASH --global-secondary-indexes $gsiJson --provisioned-throughput ReadCapacityUnits=5,WriteCapacityUnits=5 --region $REGION
    
    Write-Host "Waiting for table..." -ForegroundColor Yellow
...
if ($roleCheck) {
    ...
    $roleResult = aws iam create-role --role-name $ROLE_NAME --assume-role-policy-document file://trust-policy.json --region $REGION | ConvertFrom-Json
    $ROLE_ARN = $roleResult.Role.Arn
    Write-Host "Role created: $ROLE_ARN" -ForegroundColor Green
    
    aws iam attach-role-policy --role-name $ROLE_NAME --policy-arn arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole
    
    $dynamoPolicy = "{`"Version`":`"2012-10-17`",`"Statement`":[{`"Effect`":`"Allow`",`"Action`":[`"dynamodb:PutItem`",`"dynamodb:GetItem`",`"dynamodb:UpdateItem`",`"dynamodb:Query`",`"dynamodb:Scan`"],`"Resource`":[`"arn:aws:dynamodb:${REGION}:*:table/${TABLE_NAME}`",`"arn:aws:dynamodb:${REGION}:*:table/${TABLE_NAME}/index/*`",`"arn:aws:dynamodb:${REGION}:*:table/PostStandardJob`",`"arn:aws:dynamodb:${REGION}:*:table/PostQuickJob`"]}]}"
    $dynamoPolicy | Out-File -FilePath dynamo-policy.json -Encoding utf8
    
    aws iam put-role-policy --role-name $ROLE_NAME --policy-name DynamoDBAccess --policy-document file://dynamo-policy.json
...
Write-Host "`nStep 3: Packaging Lambda..." -ForegroundColor Yellow
...
Write-Host "`nStep 4: Deploying Lambda..." -ForegroundColor Yellow
...
# Step 5: Get User Pool
Write-Host "`nStep 5: Getting Cognito User Pool..." -ForegroundColor Yellow
...
Write-Host "`nStep 6: Creating API Gateway..." -ForegroundColor Yellow
...
if ($existingApi) {
    ...
    $apiResult = aws apigatewayv2 create-api --name $API_NAME --protocol-type HTTP --cors-configuration "AllowOrigins=*,AllowMethods=GET,POST,PUT,DELETE,OPTIONS,AllowHeaders=Content-Type,Authorization" --region $REGION | ConvertFrom-Json
    $API_ID = $apiResult.ApiId
    Write-Host "API created: $API_ID" -ForegroundColor Green
}

# Step 7: Create Authorizer
Write-Host "`nStep 7: Creating Authorizer..." -ForegroundColor Yellow
...
if ($existingAuth) {
    ...
    $authResult = aws apigatewayv2 create-authorizer --api-id $API_ID --authorizer-type JWT --identity-source '$request.header.Authorization' --name CognitoAuthorizer --jwt-configuration "Audience=,Issuer=$issuerUrl" --region $REGION | ConvertFrom-Json
...
Write-Host "`nStep 8: Creating Integration..." -ForegroundColor Yellow
...
if ($existingIntegration) {
    ...
    $sourceArn = "arn:aws:execute-api:${REGION}:*:${API_ID}/*/*"
    aws lambda add-permission --function-name $LAMBDA_NAME --statement-id apigateway-invoke --action lambda:InvokeFunction --principal apigateway.amazonaws.com --source-arn $sourceArn --region $REGION 2>$null
}

# Step 9: Create Routes
Write-Host "`nStep 9: Creating Routes..." -ForegroundColor Yellow
...
foreach ($route in $routes) {
    ...
    if ($existingRoute) {
        Write-Host "  Route exists" -ForegroundColor Gray
    } else {
        if ($route.Auth) {
            aws apigatewayv2 create-route --api-id $API_ID --route-key $routeKey --target "integrations/$INTEGRATION_ID" --authorizer-id $AUTHORIZER_ID --authorization-type JWT --region $REGION | Out-Null
        } else {
...
# Step 10: Create Stage
Write-Host "`nStep 10: Creating Stage..." -ForegroundColor Yellow
...
Write-Host "`nSave this endpoint to your frontend config!" -ForegroundColor Yellow
