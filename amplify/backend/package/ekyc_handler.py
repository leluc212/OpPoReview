...
def vnpt_ocr(b64_front, trans_token, bearer, tid, tkey):
    """
    OCR CCCD mặt trước — gửi base64 trực tiếp, KHÔNG dùng hash từ upload.
    VNPT v1 OCR nhận base64 trong imgs.img_front.
    """
    token_val = tid
    payload = {
        'imgs':           {'img_front': b64_front},
        'token':          token_val,
        'client_session': CLIENT_SESSION,
        'type':           1,
    }
    print(f'OCR with base64 length={len(b64_front)} token={token_val[:36]}')
...
def handle_ocr(event, user_id):
    ...
    POST /ekyc/ocr
    Luồng:
      1. Gọi VNPT OCR với base64 trực tiếp → lấy thông tin CCCD
      2. Upload ảnh lên VNPT file-service → lấy hash để dùng ở bước face verify
    """
    try:
    ...
    try:
        ...
        # Bước 1: OCR với base64 trực tiếp (nén < 300KB)
        print(f'Original base64 length={len(front)}')
        front_small = resize_b64(front, max_bytes=250_000, max_dim=1200)
        print(f'Resized base64 length={len(front_small)}')
        print('Calling OCR with base64...')
        ocr = vnpt_ocr(front_small, None, bearer, tid, tkey)
        ...
        # Bước 2: Upload để lấy hash cho face verify (nén < 500KB)
        print('Uploading front for face verify hash...')
        front_upload = resize_b64(front, max_bytes=400_000, max_dim=1400)
        front_hash, front_token = vnpt_upload(front_upload, bearer, tid, tkey)
        print(f'front_hash={front_hash[:60]}')
...
def _ocr_with_base64(b64_front, bearer, tid, tkey):
