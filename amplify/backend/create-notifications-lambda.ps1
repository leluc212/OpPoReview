...
try {
    aws dynamodb describe-table --table-name $TABLE_NAME --region $REGION 2>$null
...
"@
    ...
    $gsiConfig | Out-File -FilePath "gsi-config.json" -Encoding utf8
...
$DYNAMODB_POLICY | Out-File -FilePath "dynamodb-policy.json" -Encoding utf8

try {
    aws iam get-policy --policy-arn "arn:aws:iam::$(aws sts get-caller-identity --query Account --output text):policy/notifications-dynamodb-policy" 2>$null
...
aws iam attach-role-policy `
    --role-name $LAMBDA_ROLE_NAME `
    --policy-arn "arn:aws:iam::${ACCOUNT_ID}:policy/notifications-dynamodb-policy" `
    --region $REGION
...
Compress-Archive -Path "notifications-lambda.py" -DestinationPath "notifications-lambda.zip"
...
try {
    aws lambda get-function --function-name $LAMBDA_NAME --region $REGION 2>$null
